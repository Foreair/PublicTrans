...
class Station:
    ...
    #Metodo que devuelve la id de la estacion
    def Get_id(self):
        return self.id
        #Metodo que devuelve el nombre de la estacion
    def Get_name(self):
        return self.name
    #Metodo que devuelve la coordenada x de la estacion
    def Get_x(self):
        return self.x
    #Método que devuelve la coordenada y de la estacion 
    def Get_y(self):
        return self.y
    #Metodo que devuelve la linea a la que peretenece la estacion
    def Get_line(self):
        return self.line
...
#                        connections[int(destinationnation)][int(origin)]=float(j)
                    if (int(origin) in connections)==False:
                        connections[int(origin)]={}
                    if ((int(destinationnation) in connections[int(origin)]) == 0):
                        connections[int(origin)][int(destinationnation)]=float(j)
                    # as the matrix is an inferior matrix, we should duplicate the information to the superior missing part.
                    if (int(destinationnation) in connections)==False:
                        connections[int(destinationnation)]={}
                    if ((int(origin) in connections[int(destinationnation)]) == 0):
                        connections[int(destinationnation)][int(origin)]=float(j)
